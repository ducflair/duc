...
--
-- Notes:
--   - `unicode61 remove_diacritics 2` improves case/diacritic-insensitive matching.
--   - `prefix=` speeds up prefix lookups such as `for*`.
--   - `rebuild` statements at the end backfill indexes for databases that already
--     contain rows before this schema is applied.

-- FTS over element labels and descriptions.
CREATE VIRTUAL TABLE IF NOT EXISTS search_elements USING fts5(
    label,
    description,
    content='elements',
    content_rowid='rowid',
...
CREATE TRIGGER IF NOT EXISTS trg_elements_ai AFTER INSERT ON elements BEGIN
    INSERT INTO search_elements(rowid, label, description)
    VALUES (NEW.rowid, NEW.label, NEW.description);
END;
CREATE TRIGGER IF NOT EXISTS trg_elements_ad AFTER DELETE ON elements BEGIN
    INSERT INTO search_elements(search_elements, rowid, label, description)
    VALUES ('delete', OLD.rowid, OLD.label, OLD.description);
END;
CREATE TRIGGER IF NOT EXISTS trg_elements_au AFTER UPDATE OF label, description ON elements BEGIN
    INSERT INTO search_elements(search_elements, rowid, label, description)
    VALUES ('delete', OLD.rowid, OLD.label, OLD.description);
    INSERT INTO search_elements(rowid, label, description)
    VALUES (NEW.rowid, NEW.label, NEW.description);
...
CREATE VIRTUAL TABLE IF NOT EXISTS search_element_text USING fts5(
    text,
    original_text,
...
CREATE TRIGGER IF NOT EXISTS trg_element_text_ai AFTER INSERT ON element_text BEGIN
    INSERT INTO search_element_text(rowid, text, original_text)
    VALUES (NEW.rowid, NEW.text, NEW.original_text);
END;
CREATE TRIGGER IF NOT EXISTS trg_element_text_ad AFTER DELETE ON element_text BEGIN
...
END;
CREATE TRIGGER IF NOT EXISTS trg_element_text_au AFTER UPDATE OF text, original_text ON element_text BEGIN
    INSERT INTO search_element_text(search_element_text, rowid, text, original_text)
    VALUES ('delete', OLD.rowid, OLD.text, OLD.original_text);
    INSERT INTO search_element_text(rowid, text, original_text)
    VALUES (NEW.rowid, NEW.text, NEW.original_text);
...
CREATE VIRTUAL TABLE IF NOT EXISTS search_element_doc USING fts5(
    text,
    content='element_doc',
    content_rowid='rowid',
    tokenize='unicode61 remove_diacritics 2',
...
CREATE TRIGGER IF NOT EXISTS trg_element_doc_ai AFTER INSERT ON element_doc BEGIN
    INSERT INTO search_element_doc(rowid, text)
    VALUES (NEW.rowid, NEW.text);
END;
CREATE TRIGGER IF NOT EXISTS trg_element_doc_ad AFTER DELETE ON element_doc BEGIN
    INSERT INTO search_element_doc(search_element_doc, rowid, text)
    VALUES ('delete', OLD.rowid, OLD.text);
END;
CREATE TRIGGER IF NOT EXISTS trg_element_doc_au AFTER UPDATE OF text ON element_doc BEGIN
    INSERT INTO search_element_doc(search_element_doc, rowid, text)
    VALUES ('delete', OLD.rowid, OLD.text);
    INSERT INTO search_element_doc(rowid, text)
    VALUES (NEW.rowid, NEW.text);
...
CREATE VIRTUAL TABLE IF NOT EXISTS search_element_model USING fts5(
    code,
...
CREATE TRIGGER IF NOT EXISTS trg_element_model_ai AFTER INSERT ON element_model BEGIN
    INSERT INTO search_element_model(rowid, code)
    VALUES (NEW.rowid, NEW.code);
END;
CREATE TRIGGER IF NOT EXISTS trg_element_model_ad AFTER DELETE ON element_model BEGIN
    INSERT INTO search_element_model(search_element_model, rowid, code)
    VALUES ('delete', OLD.rowid, OLD.code);
END;
CREATE TRIGGER IF NOT EXISTS trg_element_model_au AFTER UPDATE OF code ON element_model BEGIN
    INSERT INTO search_element_model(search_element_model, rowid, code)
    VALUES ('delete', OLD.rowid, OLD.code);
    INSERT INTO search_element_model(rowid, code)
    VALUES (NEW.rowid, NEW.code);
...
CREATE TRIGGER IF NOT EXISTS trg_blocks_ai AFTER INSERT ON blocks BEGIN
...
CREATE TRIGGER IF NOT EXISTS trg_blocks_ad AFTER DELETE ON blocks BEGIN
    ...
    VALUES ('delete', OLD.rowid, OLD.label, OLD.description);
END;
CREATE TRIGGER IF NOT EXISTS trg_blocks_au AFTER UPDATE OF label, description ON blocks BEGIN
    INSERT INTO search_blocks(search_blocks, rowid, label, description)
    VALUES ('delete', OLD.rowid, OLD.label, OLD.description);
    INSERT INTO search_blocks(rowid, label, description)
    VALUES (NEW.rowid, NEW.label, NEW.description);
...
-- Backfill FTS indexes for databases that already contain data.
