...
The `.duc` ecosystem provides multi-language SDKs, renderers, and file format converters:

- **SDKs**:
  - [`ducjs`](/docs/typescript/overview): TypeScript/JavaScript library powered by WASM and native SQLite engines.
  - [`ducpy`](/docs/python/overview): Python SDK featuring high-level Dataclass Builders and `DucSQL` direct query execution.
  - [`ducrs`](/docs/rust/overview): Idiomatic, zero-cost Rust crate for native file manipulation.
- **Converters & Interoperability**:
  - `ducsvg`: Bidirectional SVG importer and exporter preserving vector geometry and styles.
  - `ducpdf`: High-fidelity PDF export and vector extraction package.
  - `ducdxf`: DXF CAD interchange importer and exporter.
- **Graphics Pipeline**:
  - `duc-renderer`: WebGL/WebGPU 2D rendering pipeline tailored for large-scale geometric surface visualization.