...
All three pipelines support text with custom fonts, but Build123d, ezdxf, and IfcOpenShell consume the resolved font differently. See [Using Fonts](/docs/guidelines/using-fonts#python-model-elements) for the shared `resolve_font(...)` and `FontEnum(...)` globals and the required syntax for each library.
