...
- **`pathOverrides` (`DucPath[]`)**: Overrides background and stroke properties for non-overlapping closed cycles selected by their indices in `lines`. See [Styling Linear Elements](/docs/guidelines/styling-elements#styling-linear-elements) for validation rules and selective-fill examples.
