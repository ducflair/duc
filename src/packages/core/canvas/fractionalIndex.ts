...
export const validateFractionalIndices = (
  elements: readonly DucElement[],
  {
    ...
    ignoreLogs?: true;
    reconciliationContext?: {
      localElements: ReadonlyArray<DucElement>;
      remoteElements: ReadonlyArray<DucElement>;
    };
  },
) => {
  const errorMessages = [];
  const stringifyElement = (element: DucElement | void) =>
...
export const syncMovedIndices = (
  elements: readonly DucElement[],
  movedElements: Map<string, DucElement>,
): OrderedDucElement[] => {
  try {
  ...
  return elements as OrderedDucElement[];
};
 ...
 */
const getInvalidIndicesGroups = (elements: readonly DucElement[]) => {
  const indicesGroups: number[][] = [];

  // once we find lowerBound / upperBound, it cannot be lower than that, so we cache it for better perf.
  let lowerBound: DucElement["index"] | undefined = undefined;
  let upperBound: DucElement["index"] | undefined = undefined;
  ...
  const getLowerBound = (
    index: number,
  ): [DucElement["index"] | undefined, number] => {
...
const isValidFractionalIndex = (
  index: DucElement["index"] | undefined,
  predecessor: DucElement["index"] | undefined,
  successor: DucElement["index"] | undefined,
) => {
  if (!index) {
    return false;
  }
