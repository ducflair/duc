...
import { ElementUpdate, mutateElement, newElementWith } from "../element/mutateElement";
import {
  deepCopyElement,
  duplicateElements,
  newFrameElement,
...
import { hitElementBoundingBoxOnly, hitElementBoundText, hitElementItself } from "../element/collision";
...
class App extends React.Component<AppProps, AppState> {
  ...
  private iFrameRefs = new Map<DucElement["id"], HTMLIFrameElement>();
  /**
  ...
  private initializedEmbeds = new Set<DucIframeLikeElement["id"]>();
  ...
  hitLinkElement?: NonDeletedDucElement;
  ...
  private handleEmbeddableCenterClick(element: DucIframeLikeElement) {
    if (
  ...
  private updateEmbeddables = () => {
    const iframeLikes = new Set<DucIframeLikeElement["id"]>();
  ...
  private getFrameNameDOMId = (frameElement: DucElement) => {
  ...
  public getSceneElementsMap = () => {
    return this.scene.getNonDeletedElementsMap();
  };
  
  public createGroupElem = (
    groupIdRef: string,
  ) => {
  ...
  public onInsertElements = (elements: readonly DucElement[]) => {
  ...
  public onExportImage = async (
    ...
    opts: { exportingFrame: DucFrameLikeElement | null },
  ) => {
  ...
  public onMagicframeToolSelect = () => {
    ...
    if (selectedElements.length === 0) {
      ...
      const selectedMagicFrame: DucMagicFrameElement | false =
        selectedElements.length === 1 &&
      ...
      let frame: DucMagicFrameElement;
  ...
  componentDidUpdate(prevProps: AppProps, prevState: AppState) {
    ...
    this.store.commit(elementsMap as Map<string, OrderedDucElement>, this.state);
      ...
      if (data.errorMessage) {
        ...
        const elements = (
          data.programmaticAPI
            ? convertToExcalidrawElements(
                data.elements as DucElementSkeleton[],
              )
            : data.elements
        ) as readonly DucElement[];
        ...
        if (
          ...
          const embeddables: NonDeleted<DucEmbeddableElement>[] = [];
          for (const url of embbeddableUrls) {
            const prevEmbeddable: DucEmbeddableElement | undefined =
  ...
  addElementsFromPasteOrLibrary = (opts: {
    ...
    const [gridX, gridY] = getGridPoint(dx, dy, GRID_SIZE as NullableGridSize);
  ...
  private async addElementsFromMixedContentPaste(
    ...
    if (
      ...
      const nextSelectedIds: Record<DucElement["id"], true> = {};
  ...
  scrollToContent = (
    target:
      | DucElement
      | readonly DucElement[] = this.scene.getNonDeletedElements(),
    opts?:
      | {
          fitToContent?: boolean;
          fitToViewport?: never;
      ...
      if (sceneData.storeAction && sceneData.storeAction !== StoreAction.NONE) {
        ...
        const nextCommittedElements = sceneData.elements
          ? this.store.filterUncomittedElements(
              this.scene.getElementsMapIncludingDeleted() as Map<string, OrderedDucElement>, // Only used to detect uncomitted local elements
        ...
        if (selectedElements.length === 1) {
          ...
          if (event[KEYS.CTRL_OR_CMD]) {
            ...
            if (!isTextElement(selectedElement)) {
              container = selectedElement as DucTextContainer;
            }
  ...
  toggleSnapMode = (
    ...
    return this.actionManager.executeAction(actionToggleObjectsSnapMode);
  };

  redo = (
  ) => {
    return this.actionManager.executeAction(createRedoAction(this.history, this.store));
  };

  undo = (
  ) => {
    return this.actionManager.executeAction(createUndoAction(this.history, this.store));
  };
  ...
  setElementFrameId = (
    ...
    const elem: NonDeletedDucElement | null = this.scene.getElement(activeElementId);
  ...
  setZLayerIndexAfterElement = (afterElementId: string, activeElementId:string) => {
    ...
    const afterElementIndex = elementKeys.indexOf(afterElementId);
    const activeElementIndex = elementKeys.indexOf(activeElementId);
  
    if (afterElementIndex === -1 || activeElementIndex === -1) {
      console.error("One or both elements not found");
      return; // Exit if either element is not found
    }
  
    // Calculate the differential
    const positionDifference = activeElementIndex - afterElementIndex;
  
    // Decide to move forward or backward based on the differential
    if (positionDifference >= 1) {
      // Active element needs to move backward closer to after element
      for (let i = 0; i < positionDifference; i++) {
        this.actionManager.executeAction(actionSendBackward);
      }
      // console.log("Move backward to adjust below the after element");
    } else if (positionDifference < -1) {
      // Active element is too far ahead and needs to move forward
      const forwardTimes = (positionDifference + 1)*(-1);
      for (let i = 0; i < forwardTimes; i++) {
        this.actionManager.executeAction(actionBringForward);
      }
      // console.log("Move forward to adjust just below the after element");
    } else {
      // No movement needed, or they are already adjacent
      // console.log("No adjustment needed, or they are already correctly placed");
    }
  ...
  toggleCollapseFrame = (
    ...
    const elem: DucFrameLikeElement | null = this.scene.getElement(elementId)
  ...
  public updateGroups = () => {
    ...
    const currentGroupIds = new Set(this.state.groups.map(group => group.id));
    const elementGroupIds = new Set(elements.flatMap(el => el.groupIds));
    const newGroupMap = new Map();  // Using a Map to store new groups with easy access
  
    // Detect new groups and prepare for new additions
    elements.forEach(el => {
      el.groupIds.forEach(groupId => {
        if (!currentGroupIds.has(groupId) && !newGroupMap.has(groupId)) {
    ...
    // Prepare list of groups to be removed
    const groupsToRemove = this.state.groups.filter(group => !elementGroupIds.has(group.id)).map(group => group.id);
    const groupsToAdd = Array.from(newGroupMap.values());
  
    // Update state efficiently
    this.setState(prevState => {
      const updatedGroups = prevState.groups.filter(group => !groupsToRemove.includes(group.id));
      return {
        ...prevState,
        groups: updatedGroups.concat(groupsToAdd),
      };
    });
  };
  ...
  selectElements = (ids: string[]) => {
    ...
    this.setState({
      selectedElementIds: makeNextSelectedElementIds(nextSelectedElementIds, this.state),
    });
  }
    
  // private selectElements(id: string) {
    // const nextSelectedElementIds = ids.reduce((acc, id) => {
    //   acc[id] = true;
    //   return acc;
    // }, {} as { [id: string]: true });
  ...
  private getTextElementAtPosition(
    x: number,
    y: number,
  ): NonDeleted<DucTextElement> | null {
  ...
  private getElementsAtPosition(
    ...
    const elements = (
      ...
      .filter((el) => {
        ...
        if (isIframeElement(el)) {
          iframeLikes.push(el as Ordered<DucIframeElement>);
          return false;
        }
        return true;
      })
      .concat(iframeLikes) as NonDeleted<DucElement>[];
  ...
  private startTextEditing = ({
    ...
    let existingTextElement: NonDeleted<DucTextElement> | null = null;
  ...
  private getElementLinkAtPosition = (
    ...
    hitElement: NonDeletedDucElement | null,
  ): DucElement | undefined => {
  ...
  private getTopLayerFrameAtSceneCoords = (sceneCoords: {
    ...
    const frames = this.scene
      ...
      .filter((frame): frame is DucFrameLikeElement =>
    ...
    if (this.state.multiElement) {
      ...
      if (lastPoint === lastCommittedPoint) {
        ...
        const [gridX, gridY] = getGridPoint(
          scenePointerX,
          scenePointerY,
          event[KEYS.CTRL_OR_CMD] ? null : GRID_SIZE as NullableGridSize,
        );
  ...
  private handleEraser = (
    ...
    const processElements = (elements: DucElement[]) => {
  ...
  private handleCanvasPointerDown = (
    ...
    if (
      ...
      const element = this.state.newElement as DucFreeDrawElement;
      this.updateScene({
    ...
    if (this.state.activeTool.type === "text") {
      ...
      this.setState({
        newElement: pendingImageElement as DucNonSelectionElement,
          ...
          if (!this.state.selectedElementIds[hitElement.id]) {
            ...
            if (
              ...
              this.setState((prevState) => {
                ...
                const previouslySelectedElements: DucElement[] = [];
  ...
  private isASelectedElement(hitElement: DucElement | null): boolean {
  ...
  private handleTextOnPointerDown = (
    ...
    if (hasBoundTextElement(element)) {
      container = element as DucTextContainer;
  ...
  private handleFreeDrawElementOnPointerDown = (
    event: React.PointerEvent<HTMLElement>,
    elementType: DucFreeDrawElement["type"],
    pointerDownState: PointerDownState,
  ) => {
  ...
  private getCurrentItemRoundness(
    ...
    // TODO: In the future get the roundness actually working
    return null
  ...
  private getNumLastElementOfType = (type: DucElement["type"]) => {
    return this.getSceneElementsIncludingDeleted().filter((element) => element.type === type).length+1;
  }
  ...
  private createFrameElementOnPointerDown = (
    ...
    const elementType = type === TOOL_TYPE.magicframe ? "magicframe" : "frame";
    ...
    return withBatchedUpdates((childEvent: PointerEvent) => {
      ...
      if (pointerDownState.resize.isResizing) {
        ...
        const selectedFrames = this.scene
          ...
          .filter((element): element is DucFrameLikeElement =>
  ...
  private updateImageCache = async (
    elements: readonly InitializedDucImageElement[],
    files = this.files,
  ) => {
  ...
  private addNewImagesToImageCache = async (
    imageElements: InitializedDucImageElement[] = getInitializedImageElements(
      this.scene.getNonDeletedElements(),
    ),
  ...
  private maybeSuggestBindingsForLinearElementAtCoords = (
    ...
    // into `linearElement`
    oppositeBindingBoundElement?: DucBindableElement | null,
  ): void => {
    ...
    const suggestedBindings = pointerCoords.reduce(
      (acc: NonDeleted<DucBindableElement>[], coords) => {
        const hoveredBindableElement = getHoveredElementForBinding(
          coords,
          this.scene.getNonDeletedElements(),
          this.scene.getNonDeletedElementsMap(),
  ...
  private clearSelection(hitElement: DucElement | null): void {
    this.setState((prevState) => ({
      selectedElementIds: makeNextSelectedElementIds({}, prevState),
      activeEmbeddable: null,
      selectedGroupIds: {},
      ...
      if (
        ...
        this.setState({
          elementsToHighlight: getElementsInResizingFrame(
            this.scene.getNonDeletedElements(),
            draggingElement as DucFrameLikeElement,
            this.state,
            this.scene.getNonDeletedElementsMap(),
          ),
        });
  ...
  private maybeHandleResize = (
    ...
    const selectedFrames = selectedElements.filter(
      (element): element is DucFrameLikeElement =>
        isFrameLikeElement(element),
    );
    ...
    if (
      ...
      const elementsToHighlight = new Set<DucElement>();
      selectedFrames.forEach((frame) => {
        getElementsInResizingFrame(
          this.scene.getNonDeletedElements(),
          frame,
