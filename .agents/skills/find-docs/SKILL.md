...
description: >-
  ...
  developer technology. Use this skill whenever the user asks about Context7 or a specific
...
Run commands with `npx ctx7` so setup always uses the latest CLI without a global install:
...
npx ctx7 library "Next.js" "How to set up app router with middleware"
...
If Context7 does not index the exact library, use the closest official entry
only for shared platform behavior, then verify package-specific APIs against
the library's official repository and package registry.
