...
fn get_schema_version_from_fbs(fbs_file_path: &PathBuf) -> Result<i8, Box<dyn std::error::Error>> {
    ...
    if let Some(line_result) = reader.lines().next() {
        ...
        if line.starts_with(PREFIX) {
            ...
            return Ok(version_str.parse::<i8>()?);
            ...
            // Take the first part after '=', assuming it's the number
    ...
    eprintln!("cargo:warning=Could not parse schema version from {:?}. Defaulting to 0.", fbs_file_path);
    Ok(0)
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match get_schema_version_from_fbs(&fbs_path) {
        ...
        Err(e) => {
            eprintln!("cargo:warning=Failed to read or parse schema version from {:?}: {}. Defaulting to 0.", fbs_path, e);
            println!("cargo:rustc-env=DUC_SCHEMA_VERSION=0");
        }
    }
