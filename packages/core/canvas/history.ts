...
class History {
  private elementCache = new Map<string, Map<number, DucElement>>();
  ...
  private dehydrateHistoryEntry({
    ...
    return {
      ...
      elements: elements.map((element: DucElement) => {
  ...
  pushEntry(appState: AppState, elements: readonly DucElement[]) {
   ...
   *  it.
   */
  setCurrentState(appState: AppState, elements: readonly DucElement[]) {
  ...
  record(state: AppState, elements: readonly DucElement[]) {
    if (this.recording) {
