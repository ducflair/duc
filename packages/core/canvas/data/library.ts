...
export const useHandleLibrary = (
  ...
  const { ducAPI: excalidrawAPI } = opts;
      ...
      const unsubOnLibraryUpdate = onLibraryUpdateEmitter.on(
        async (update, nextLibraryItems) => {
          ...
          try {
            ...
            if (isLoaded && optsRef.current.ducAPI) {
              optsRef.current.ducAPI.canvas.updateScene({
                appState: {
