  ...
  label?: {
    ...
    | (
        ...
        | ((
            ...
            | {
                type?: "text";
                id: DucTextElement["id"];
                text: string;
              }
          ) &
            Partial<DucTextElement>)
      ) &
        MarkOptional<ElementConstructorOpts, "x" | "y">;
  start?:
    | (
        ...
        MarkOptional<ElementConstructorOpts, "x" | "y">;
} & Partial<DucLinearElement>;

export type ValidContainer =
  | {
      type: Exclude<DucGenericElement["type"], "selection">;
      id?: DucGenericElement["id"];
      label?: {
        text: string;
        fontSize?: number;
...
export type DucElementSkeleton =
  | Extract<
      Exclude<DucElement, DucSelectionElement>,
      DucIframeLikeElement | DucFreeDrawElement
    >
  | ({
      type: Extract<DucLinearElement["type"], "line">;
      x: number;
      y: number;
    } & Partial<DucLinearElement>)
  | ValidContainer
  ...
  | ({
      type: Extract<DucImageElement["type"], "image">;
      x: number;
      y: number;
      fileId: FileId;
    } & Partial<DucImageElement>)
  | ({
      type: "frame";
      children: readonly DucElement["id"][];
      name?: string;
    } & Partial<DucFrameElement>)
  | ({
      type: "magicframe";
      children: readonly DucElement["id"][];
      name?: string;
    } & Partial<DucMagicFrameElement>);
...
const bindTextToContainer = (
  ...
  const textElement: DucTextElement = newTextElement({
    x: 0,
    y: 0,
...
const bindLinearElementToElement = (
  linearElement: DucArrowElement,
  start: ValidLinearElement["start"],
  end: ValidLinearElement["end"],
  elementStore: ElementStore,
  elementsMap: NonDeletedSceneElementsMap,
): {
  linearElement: DucLinearElement;
  startBoundElement?: DucElement;
  endBoundElement?: DucElement;
} => {
  ...
  if (start) {
    ...
    if (startType) {
      ...
      bindLinearElement(
        linearElement,
        startBoundElement as DucBindableElement,
        "start",
        elementsMap,
      );
    }
  ...
  if (end) {
    ...
    if (endType) {
      ...
      bindLinearElement(
        linearElement,
        endBoundElement as DucBindableElement,
        "end",
        elementsMap,
      );
    }
...
class ElementStore {
  excalidrawElements = new Map<string, DucElement>();
...
export const convertToExcalidrawElements = (
  elementsSkeleton: DucElementSkeleton[] | null,
  ...
  const elementsWithIds = new Map<string, DucElementSkeleton>();
  const oldToNewElementIdMap = new Map<string, string>();

  // Create individual elements
  for (const element of elements) {
    let excalidrawElement: DucElement;
        ...
        if (element.label?.text) {
          ...
          switch (element.type) {
            case "arrow": {
              ...
              const { linearElement, startBoundElement, endBoundElement } =
                bindLinearElementToElement(
                  excalidrawElement as DucArrowElement,
                  start,
                  end,
                  elementStore,
                  elementsMap,
  ...
  for (const [id, element] of elementsWithIds) {
    ...
    const childrenElements: DucElement[] = [];
