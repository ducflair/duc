...
export const decodePngMetadata = async (blob: Blob) => {
  ...
  if (metadata?.keyword === MIME_TYPES.duc) {
    try {
      ...
      if (!("encoded" in encodedData)) {
        ...
        if (
          "type" in encodedData &&
          encodedData.type === EXPORT_DATA_TYPES.duc
        ) {
...
export const encodeSvgMetadata = async ({ text }: { text: string }) => {
  ...
  let metadata = "";
  metadata += `<!-- payload-type:${MIME_TYPES.duc} -->`;
...
export const decodeSvgMetadata = async ({ svg }: { svg: string }) => {
  if (svg.includes(`payload-type:${MIME_TYPES.duc}`)) {
