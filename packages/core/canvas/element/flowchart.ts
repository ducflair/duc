...
import type { AppState, PendingDucElements, Point } from "../types";
...
const getNodeRelatives = (
  ...
  node: DucBindableElement,
  elementsMap: ElementsMap,
  direction: LinkDirection,
) => {
  const items = [...elementsMap.values()].reduce(
    (acc: { relative: DucBindableElement; heading: Heading }[], el) => {
      let oppositeBinding;
      if (
        isElbowArrow(el) &&
...
const getOffsets = (
  element: DucFlowchartNodeElement,
  linkedNodes: DucElement[],
  direction: LinkDirection,
) => {
...
const addNewNode = (
  ...
  invariant(
    ...
    "not an DucFlowchartNodeElement",
  );
...
export const addNewNodes = (
  startNode: DucFlowchartNodeElement,
  elementsMap: ElementsMap,
  appState: AppState,
  direction: LinkDirection,
...
const createBindingArrow = (
  startBindingElement: DucFlowchartNodeElement,
  endBindingElement: DucFlowchartNodeElement,
  elementsMap: ElementsMap,
  direction: LinkDirection,
  appState: AppState,
) => {
  ...
  const changedElements = new Map<string, OrderedDucElement>();
  changedElements.set(
    startBindingElement.id,
    startBindingElement as OrderedDucElement,
  );
  changedElements.set(
    endBindingElement.id,
    endBindingElement as OrderedDucElement,
  );
  changedElements.set(
    bindingArrow.id,
    bindingArrow as OrderedDucElement,
  );
...
export class FlowChartNavigator {
  ...
  private sameLevelNodes: DucElement[] = [];
  ...
  private visitedNodes: Set<DucElement["id"]> = new Set();
...
export class FlowChartCreator {
  ...
  pendingNodes: PendingDucElements | null = null;
