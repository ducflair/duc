...
use duc::types::{ElementBackground, ElementContentBase, DucPath};
...
impl PdfBackgroundRenderer {
    ...
    pub fn apply_background_color(background: &ElementBackground) -> ConversionResult<Vec<Operation>> {
        let mut ops = Vec::new();
    ...
    pub fn render_linear_element_backgrounds(
        ...
        let override_map: std::collections::HashMap<BTreeSet<usize>, &ElementBackground> = 
            path_overrides
                .iter()
                .filter_map(|p| {
                    p.background.as_ref().map(|bg| {
                        (p.line_indices.iter().map(|&i| i as usize).collect(), bg)
                    })
                })
                .collect();
        ...
        for (path_ops, line_indices) in paths_with_indices {
            ...
            let background = override_map.get(&indices_set).copied().unwrap_or(primary_background);
