...
impl ElementStreamer {
    ...
    /// Stream elements within specified bounds with plot-relative coordinates for PLOT mode
    pub fn stream_elements_within_bounds_with_plot_offset(
        &mut self,
        elements: &[ElementWrapper],
        bounds: (f64, f64, f64, f64),
        plot_bounds: Option<(f64, f64, f64, f64)>, // Plot bounds for relative positioning
        local_state: Option<&duc::types::DucLocalState>,
        resource_streamer: &mut ResourceStreamer,
        block_manager: &mut BlockManager,
        hatching_manager: &mut HatchingManager,
        ...
        for element_wrapper in filtered_elements {
            ...
            let has_clipping = base.frame_id.is_some();
            if let Some(frame_id) = &base.frame_id {
                let clipping_ops = self.handle_frame_clipping_with_plot_offset(frame_id, elements, bounds, plot_bounds)?;
                all_operations.extend(clipping_ops);
            }

            // Stream the element with plot-relative coordinates
            let element_ops = self.stream_element_with_resources_and_plot_offset(
                &element_wrapper.element,
                plot_bounds,
                local_state,
                document,
            ...
            println!(
                "DEBUG: Element {} with plot offset generated {} operations",
                base.id,
                element_ops.len()
            );
            all_operations.extend(element_ops);
    ...
    /// Stream a single element with resource managers and local state using plot-relative coordinates
    fn stream_element_with_resources_and_plot_offset(
        &mut self,
        element: &DucElementEnum,
        ...
        if !is_pdf {
            ...
            // Apply transformation (position, rotation) with plot-relative coordinates
            let base = Self::get_element_base(element);
            if base.x != 0.0 || base.y != 0.0 || base.angle != 0.0 {
                let transform_ops = self.create_transformation_matrix_with_plot_offset(
                    base.x, base.y, base.angle, plot_bounds, local_state
                );
                operations.extend(transform_ops);
            }
        ...
        let element_ops = match element {
            ...
            DucElementEnum::DucPdfElement(pdf) => self.stream_pdf_element_with_plot_offset(pdf, document, pdf_embedder, plot_bounds)?,
            DucElementEnum::DucImageElement(image) => {
                self.stream_image_with_plot_offset(image, document, pdf_embedder, image_manager, resource_streamer, plot_bounds)?
            }
            DucElementEnum::DucBlockInstanceElement(block_instance) => {
                self.stream_block_instance_with_plot_offset(block_instance, block_manager, plot_bounds)?
            }
            DucElementEnum::DucFrameElement(frame) => self.stream_frame_with_plot_offset(frame, plot_bounds)?,
            DucElementEnum::DucPlotElement(plot) => self.stream_plot_with_plot_offset(plot, plot_bounds)?,
    ...
    /// Handle frame clipping for elements with plot-relative coordinates
    fn handle_frame_clipping_with_plot_offset(
        &self,
        frame_id: &str,
        elements: &[ElementWrapper],
        _bounds: (f64, f64, f64, f64),
        ...
        if let Some(frame_wrapper) = elements.iter().find(|wrapper| {
            ...
            if let DucElementEnum::DucFrameElement(frame) = &frame_wrapper.element {
                if frame.stack_element_base.clip {
                    // Get frame position and dimensions
                    let frame_base = &frame.stack_element_base.base;
                    let mut x = frame_base.x;
                    let mut y = frame_base.y;
                    let width = frame_base.width;
                    let height = frame_base.height;

                    // Apply plot-relative offset if we have plot bounds
                    if let Some((plot_x, plot_y, _, _)) = plot_bounds {
                        x = x - plot_x;
                        y = y - plot_y;
                    }
                    ...
                    if x != 0.0 || y != 0.0 {
                        ops.push(Operation::new(
                            &format!("% Applying frame transformation with plot offset: x={}, y={}", x, y),
                            vec![],
                        ));
                    }
                    ...
                    ops.push(Operation::new(
                        "re",
                        vec![
                            Object::Real(x as f32),      // Frame X position (plot-relative)
                            Object::Real(y as f32),      // Frame Y position (plot-relative)
                            Object::Real(width as f32),  // Frame width
                            Object::Real(height as f32), // Frame height
                        ],
                    ));
                    ...
                    ops.push(Operation::new(
                        &format!(
                            "% Clipping active for frame: {} (plot-relative bounds: x={}, y={}, w={}, h={})",
                            frame_id, x, y, width, height
                        ),
                        vec![],
                    ));
    ...
    /// Create transformation matrix operations with plot-relative coordinates
    /// In PLOT mode, elements should be positioned relative to their containing plot's north-west corner
    pub fn create_transformation_matrix_with_plot_offset(
        &self,
        x: f64,
        y: f64,
        angle: f64,
        ...
        // If we have plot bounds, calculate relative position within the plot
        let (final_x, final_y) = if let Some((plot_x, plot_y, _, _)) = plot_bounds {
            // In PLOT mode, the element's position should be relative to the plot's north-west corner
            // Subtract the plot's north-west corner (plot_x, plot_y) from the element's position
            let relative_x = x - plot_x;
            let relative_y = y - plot_y;

            // Apply scroll offset to the relative coordinates
            (relative_x + scroll_x, relative_y + scroll_y)
        } else {
            // No plot bounds, use regular world coordinates with scroll offset
            (x + scroll_x, y + scroll_y)
        };

        self.create_transformation_matrix(final_x, final_y, angle)
    }
    ...
    /// Stream PDF element with plot-relative coordinates
    fn stream_pdf_element_with_plot_offset(
        &mut self,
        pdf: &DucPdfElement,
        document: &mut Document,
        pdf_embedder: &mut PdfEmbedder,
        ...
        // Calculate plot-relative position if we have plot bounds
        let (position_x, position_y) = if let Some((plot_x, plot_y, _, _)) = plot_bounds {
            (pdf.base.x - plot_x, pdf.base.y - plot_y)
        } else {
            (pdf.base.x, pdf.base.y)
        };

        // Build embed options with plot-relative positioning
        let options = EmbedOptions {
            page_range: Some(PageRange::Single(0)),
        ...
        match pdf_embedder.embed_pdf(document, &embed_id, &options) {
            ...
            Err(e) => {
                ...
                ops.push(Operation::new(
                    "re",
                    vec![
                        Object::Real(position_x as f32),
                        Object::Real(position_y as f32),
                        Object::Real(pdf.base.width as f32),
                        Object::Real(pdf.base.height as f32),
                    ],
                ));
    ...
    /// Stream image element with plot-relative coordinates
    fn stream_image_with_plot_offset(
        &mut self,
        image: &DucImageElement,
        document: &mut Document,
        pdf_embedder: &mut PdfEmbedder,
        ...
        // Calculate plot-relative position if we have plot bounds
        let (position_x, position_y) = if let Some((plot_x, plot_y, _, _)) = plot_bounds {
            (image.base.x - plot_x, image.base.y - plot_y)
        } else {
            (image.base.x, image.base.y)
        };
        ...
        if let Some(file_id) = &image.file_id {
            ...
            if self.context_has_embedded_pdf(file_id) {
                ...
                if let Some(&image_id) = self.images.get(file_id) {
                    ...
                    // Use image_manager to draw the image with plot-relative positioning
                    ops.extend(hipdf::images::ImageManager::draw_image(
                        &resource_name,
                        position_x as f32,
                        position_y as f32,
    ...
    /// Stream block instance element with plot-relative coordinates
    fn stream_block_instance_with_plot_offset(
        &self,
        block_instance: &DucBlockInstanceElement,
        _block_manager: &mut BlockManager,
        _plot_bounds: Option<(f64, f64, f64, f64)>,
    ) -> ConversionResult<Vec<Operation>> {
        let mut ops = Vec::new();
        ...
        // For now, create a placeholder for the block instance
        ops.push(Operation::new("% Block Instance with plot offset", vec![]));
        ops.push(Operation::new(
            "re",
            vec![
                Object::Real(0.0),
        ...
        // TODO: Use block_manager to render actual block content with plot offset
        // block_manager.render_block(block_id, &block_instance.attributes)?;

        Ok(ops)
    }

    /// Stream frame element with plot-relative coordinates
    fn stream_frame_with_plot_offset(
        &self,
        ...
        // TODO: Stream child elements within the frame with plot offset
        ops.push(Operation::new(
            "% TODO: Stream frame child elements with plot offset",
            vec![],
        ));
    ...
    /// Stream plot element with plot-relative coordinates
    fn stream_plot_with_plot_offset(
        &self,
        plot: &DucPlotElement,
        _plot_bounds: Option<(f64, f64, f64, f64)>,
    ) -> ConversionResult<Vec<Operation>> {
        ...
        if plot.stack_element_base.stack_base.is_plot {
            ...
            // TODO: Stream child elements within the clipping bounds with plot offset
            ops.push(Operation::new(
                "% TODO: Stream plot child elements with plot offset",
                vec![],
            ));
